...
public class QrCodePayment extends Payment {
    ...
    public void generateReceipt(Order order) {
        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss");// to set the format of the
                                                                                         // date and time
        System.out.println("Order ID\t: " + order.getOrderId());
        System.out.println("Payment Method\t: " + getPaymentMethod());
        System.out.println(
                "Amount\t\t: RM " + JLineMenu.GREEN + String.format("%.2f", order.getGrandTotal()) + JLineMenu.RESET);
        ...
        for (OrderItem item : order.getItems()) {
            System.out.printf("%-45s %-10d RM%-8.2f%n", item.getProduct().getName(), item.getQuantity(),
                    item.getSubtotal());
        }
    ...
    public void generatePrintableReceipt(Order order) {
        ...
        try {
            ...
            try ( // write to file
                ...
                writeReceipt.write("============================================\n");
                DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss");// to set the format of
                                                                                                 // the date and time
